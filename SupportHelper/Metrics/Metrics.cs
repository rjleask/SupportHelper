...
namespace SupportHelper
{
    ...
    public class Metrics : DataTable
    {
        ...
        private void LogSubmission(MultipartFormDataContent submission)
        {
            ...
            log.AppendLine(string.Format("Endpoint: {0}", Endpoint));

            log.AppendLine(string.Format("Disabled metrics: {0}", ValidationHelper.GetString(SettingsHelper.AppSettings["SHDisabledMetrics"], string.Empty)));

            var metricsProcessor = new MetricsProcessor(submission);

            foreach (var field in metricsProcessor.FormFields)
            {
                log.AppendLine(string.Format("{0}: {1}", ResHelper.GetString(field.Key), field.Value.Value));
            }
